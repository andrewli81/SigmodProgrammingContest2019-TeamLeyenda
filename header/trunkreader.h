...
//
#ifndef TRUNKREADER_H
#define TRUNKREADER_H
#include <string>
#include <vector>
#include <fstream>
#include <cstring>
...
private:
    ...
    void readFileBlockAndSeparate(
        ...
        while (queue.try_dequeue_from_producer(token, task)) {
            ...
            assert(pread(fd, readBuffer, blockSize, offset + blockNumber * kBlockSize) != -1);
    ...
    void readFileBlockAndSeparate2(
            char*& valueStore,
            char* readBuffer,
            moodycamel::ConcurrentQueue<ReadTask>& queue,
            moodycamel::ProducerToken& token) {
        ...
        while (queue.try_dequeue_from_producer(token, task)) {
            ...
            auto blockCount = (blockSize / TUPLE_SIZE);
            copy(readBuffer,
                 readBuffer + blockCount * TUPLE_SIZE,
                 valueStore + countOffset * TUPLE_SIZE);
        }
        assert(close(fd) != -1);
    }
    ...
    uint64_t nThreads;
public:
    TrunkReader(string infile, uint64_t nThreads, uint64_t offset, uint64_t size)
    : infile(std::move(infile)), size(size), nThreads(nThreads), offset(offset) {
    }
    void read(char*& keyStore, char*& valueStore) {
        ...
        for (uint64_t i = 0; i < nThreads; i++) {
            ioThreads.emplace_back(&TrunkReader::readFileBlockAndSeparate, this,
    ...
    void read(char* valueStore) {
        ...
        ioThreads.reserve(nThreads);
        for (uint64_t i = 0; i < nThreads; i++) {
            ioThreads.emplace_back(&TrunkReader::readFileBlockAndSeparate2, this,
                                   ref(valueStore),
                                   static_cast<char*>(readBuffer) + i*kBlockSize,
                                   ref(queue), ref(token));
        }
